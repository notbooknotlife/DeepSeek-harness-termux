...
| **3** | **开启局域网**：先预检（未开启→可继续；已开启→显示端口并返回）；风险确认（`y` 确认/其他返回）→ 输转发端口（0返回/3080不可用/默认8080）→ 检测装 caddy → 生成 Caddyfile 并启动 → 显示本机/局域网地址，**须重启 dsh（选项1）生效** |
| **4** | **局域网状态**：显示转发端口 + caddy 状态；输 `y` 一键关闭（还原 dsh + 停 caddy + 删配置），其他返回菜单 |
| **5** | 查看 DSH 服务/安装状态，回车返回菜单 |
| **6** | 完全卸载（**6 确认卸载并退出菜单/0 返回菜单/其他无效**） |
| **0** | 退出菜单 |

### 局域网访问（选项3）
dsh 官方**仅允许本机监听 `127.0.0.1`**（安全限制，不支持 `0.0.0.0`）。局域网访问通过 **caddy 反向代理**实现：caddy 在转发端口监听，反代到 dsh 本机 `127.0.0.1:3080`；同时注入 dsh 白名单/鉴权（备份原文件），放行局域网 WebSocket 与核心设置接口。其它设备通过 `http://<本机IP>:<转发端口>` 访问。
- 本机始终 `127.0.0.1:3080`；转发端口默认 `8080`，不可用 `3080`，`0` 返回。
- 开启会**备份并改动 dsh 的 `lib/index.js`**；关闭（选项4 输 y）会**还原 dsh + 停 caddy + 删配置**。
- 预检：dsh 与备份一致（未开）可继续开启；不一致（已开）提示已开启并显示端口。
- ⚠ 局域网会对同 Wi-Fi 设备开放，且部分 dsh 敏感接口放行；**dsh 升级会覆盖注入**，需重开。第三方插件接口可能仍 403（属插件自身）。
