...
| **3** | **局域网启动**：用 **socat 端口转发**，让同 Wi-Fi 其它设备访问本机 dsh。输端口(0返回/不可用3080/默认8080)，自动安装并启动 socat 转发到 `127.0.0.1:3080`；测试完返回菜单自动停止 socat |
| **4** | 查看 DSH 服务/安装状态，回车返回菜单 |
| **5** | 完全卸载（**5 确认卸载并退出菜单/0 返回菜单/其他无效**） |
| **0** | 退出菜单 |

### 局域网启动（选项3）
dsh 官方**仅允许本机监听 `127.0.0.1`**（安全限制，不支持 `0.0.0.0`）。局域网访问通过 **socat 端口转发**实现：socat 在选定端口监听，转发到 dsh 本机 `127.0.0.1:3080`。其它设备通过 `http://<本机IP>:<转发端口>` 访问。
- 端口默认 `8080`，不可用 `3080`（与 dsh 本体冲突），`0` 返回菜单。
- 自动安装 socat（`pkg install socat`）；返回菜单自动终止 socat。
- ⚠ dsh 服务 + socat 均随 Termux 进程存活，Termux 关闭后局域网同步失效。
