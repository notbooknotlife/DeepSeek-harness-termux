...
| **3** | 局域网启动（需先经**5 开启局域网**）；测试模式，回车返回时关端口 |
| **4** | 查看 DSH 服务/安装状态，回车返回菜单 |
| **5** | 局域网**开/关**（默认关）。按当前状态提示开启或关闭；**输入 `y` 确认，其他任意键返回菜单**；改动需**重启 dsh** 生效 |
| **6** | 完全卸载（**6 确认卸载并退出菜单/0 返回菜单/其他无效**） |
| **0** | 退出菜单 |

### 局域网开/关（选项5）
默认**关闭**（仅本机 `127.0.0.1`）。开启后同一 Wi-Fi 设备可通过 `http://<本机IP>:<端口>` 访问。
**⚠ 安全提示：局域网模式下同网内其他设备也能访问 dsh（官方警告存在远程代码执行 RCE 风险），请仅在可信网络开启。**
改动 dsh 启动配置后需**重启 dsh** 生效。
