...
> 4.想还原官方源，在列表里选 **`Mirrors by Termux (default repository)`** 即可。
...
> **🇨🇳 国内用户（Gitee 镜像）**：大陆网络拉 GitHub raw 常被限速/超时。可用 **Gitee 镜像** 一键安装——同一份代码、稳定直连、不走 GitHub：
>
> ```sh
...
| **3** | **开启局域网**：预检（`client.js` 未注入→可开启；已注入→显示"局域网已开启"+拉起 dsh(3080)与 caddy）→ 风险确认(y/其他返回) → 输转发端口(0返回/3080不可用/默认8080) → 注入 client.js + 启动 caddy → 显示本机/局域网地址；**回车返回菜单自动关闭 caddy** |
| **4** | **局域网状态**：显示转发端口 + caddy 状态；输 `y` 一键关闭（**还原 client.js** + 停 caddy + 删配置），其他返回菜单 |
| **5** | 查看 DSH 服务/安装状态，回车返回菜单 |
| **6** | 完全卸载（**6 确认卸载并退出菜单/0 返回菜单/其他无效**） |
| **0** | 退出菜单 |

### 局域网访问（选项3）
dsh 官方**仅允许本机监听 `127.0.0.1`**（安全限制，不支持 `0.0.0.0`）。局域网访问通过 **caddy 反向代理**实现：caddy 在转发端口监听并**改写 Host/Origin/Referer 为 `localhost`**（`header_up`），反代到 dsh 本机 `127.0.0.1:3080`；同时**注入 `client.js`**（`crypto.randomUUID`→`randomUuid`），解决局域网非 secure context 下前端 id 生成失败。其它设备通过 `http://<本机IP>:<转发端口>` 访问，预览设置等接口正常。
- 本机始终 `127.0.0.1:3080`；转发端口默认 `8080`，不可用 `3080`，`0` 返回；**不自动打开浏览器**。
- 预检按 `client.js` 是否已注入（含 `randomUuid()`）判定：未注入→可开启；已注入→已开启（拉起 3080 与 caddy）。
- 关闭（选项4 输 y）会**还原 client.js** + 停 caddy + 删配置。
- ⚠ 局域网会对同 Wi-Fi 设备开放；**dsh 升级会覆盖注入**，需重开。
